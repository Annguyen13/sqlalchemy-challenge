...
def start(start=None):
 # Create our session (link) from Python to the DB
    session = Session(engine)
    start_dt =start
    start_data = session.query(Measurement.date, func.min(Measurement.tobs), func.max(Measurement.tobs), func.avg(Measurement.tobs)).filter(Measurement.date >= start_dt).all()
    start_list = []
    for stdate in start_data:
        start_dic = {}
        start_dic["Date"] = stdate[0]
        start_dic["MinTobs"] = stdate[1]
        start_dic["MaxTobs"] = stdate[2]
        start_dic["AvgTobs"] = stdate[3]
...
def start_end(start, end):
 ...
 # Create our session (link) from Python to the DB
    ...
    start_end_list = []
    for station, min, max, avg in start_end_data:
        start_end_dict = {}
